...
struct PS_OUTPUT
{
    ...
    float4 ao : SV_Target1;
    float4 w_pos : SV_Target2;
    float4 w_normal : SV_Target3;
...
PS_OUTPUT main(PS_INPUT input)
{
    ...
    float r = texture_arr_s.Sample(sampler_linear, uvw).g;
    float m = texture_arr_s.Sample(sampler_linear, uvw).r;
    float ao = texture_arr_s.Sample(sampler_linear, uvw).b;
    output.roughness = float4(r, r, r, 1);
    output.metallic = float4(m, m, m, 1);
    output.ao = float4(ao, ao, ao, 1);
    output.color = 
